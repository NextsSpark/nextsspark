...
export async function POST(_request: Request, { params }: Params) {
  try {
    ...
    const qrCode = buildQrCodeUrl(publicUrl);

    record.publicUrl = publicUrl;
    record.qrCode = qrCode;
    await record.save();

    return NextResponse.json({ success: true, message: 'QR regenerated.', record: { publicUrl, qrCode } });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ success: false, message: 'Unable to regenerate QR.' }, { status: 500 });
  }
}
